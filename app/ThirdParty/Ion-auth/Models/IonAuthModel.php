...
class IonAuthModel
{
	...
	/**
	 * проверка наличия пользователя с таким chat_id
	 *
	 * @param string $identity Identity
	 *
	 * @return boolean
	 * @author Mathew
	 */
	public function idCheck(string $identity=''): bool
	{
		$this->triggerEvents('identity_check');
		...
		$builder = $this->db->table($this->tables['users']);
		return $builder->where('chat_id', $identity)
					   ->limit(1)
					   ->countAllResults() > 0;
	}
	...
	public function register(string $identity, string $password, string $email, array $additionalData=[], array $groups=[])
	{
		...
		if (isset($userData['first_name'])) {
			$userData['first_name'] = json_encode($userData['first_name']);
		}
		if (isset($userData['last_name'])) {
			$userData['last_name'] = json_encode($userData['last_name']);
		}
		if (isset($userData['chat_id'])) {
			$userData['id'] = $userData['chat_id'];
		}
	...
	public function update(int $id, array $data): bool
	{
		...
		if ($this->identityCheck($data[$this->identityColumn]) AND $user->{$this->identityColumn} !== $data[$this->identityColumn])
		{
	...
	public function deleteUser(int $id): bool
	{
		...
		if ($this->db->transStatus() === false)
		{
			...
			$this->setError('IonAuth.account_delete_unsuccessful');
			return false;
		}
		...
		$this->setMessage('IonAuth.account_delete_successful');
		return true;
	}
